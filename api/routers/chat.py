...
from core.streaming import get_sse_headers
...
class ChatMessageResponse(BaseModel):
    ...
    created_at: str
    conversation_id: str
    user_message_id: int = None  # ID of the user message that triggered this response
    ...
    async def generate_stream():
        try:
            ...
            if data.content.strip():
                ...
                yield sse_service.format_custom_event('user_message', {'message': user_result['message']})
            
            # Prepare conversation messages using ChatService
            base_messages = await chat_service.prepare_conversation_messages(conversation_id)
            
            # Stream events using SSE service for consistent formatting
            async for sse_event in sse_service.stream_service_events(
                chat_service.get_streaming_generator(conversation_id, base_messages),
        ...
        if data.use_streaming:
            ...
            async def regenerate_stream():
                try:
                    # Stream events using SSE service for consistent formatting
                    async for sse_event in sse_service.stream_service_events(
                        chat_service.get_streaming_generator(
                            conversation_id=data.conversation_id,
                            messages_for_api=messages_for_api,
                            is_regeneration=True,
                            message_id_to_replace=data.message_id_to_replace
                        ),
                        error_prefix="Regeneration error"
                    ):
                        yield sse_event
                except Exception as e:
